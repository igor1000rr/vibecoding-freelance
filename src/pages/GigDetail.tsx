...
export default function GigDetail() {
  ...
  const packageLabels: Record<PackageKey, string> = {
    economy: t('gig.economy'), standard: t('gig.standard'), premium: t('gig.premium'),
  };
        ...
        <div className="flex-1 min-w-0 space-y-8">
          <div className="space-y-3">
            <div className="relative aspect-video rounded-2xl overflow-hidden bg-nebula">
              <img src={gig.images[currentImage]} alt={gig.title} className="w-full h-full object-cover" />
              {gig.images.length > 1 && (
                <>
              ...
              <div className="flex gap-2">
                {gig.images.map((img, i) => (
                  <button key={i} onClick={() => setCurrentImage(i)}
                    className={`w-20 h-14 rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${i === currentImage ? 'border-gold' : 'border-gold/20 opacity-60 hover:opacity-100'}`}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gold/20">
                    <th className="text-left py-3 px-4 text-sm text-muted font-medium">{t('gig.package')}</th>
                    {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                      <th key={key} className="text-center py-3 px-4">
                        <span className={`text-sm font-semibold ${key === activePackage ? 'text-gold' : 'text-heading'}`}>{packageLabels[key]}</span>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-gold/20">
                    <td className="py-3 px-4 text-sm text-muted">{t('gig.price')}</td>
                    {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                      <td key={key} className="text-center py-3 px-4 font-mono text-sm font-bold text-gold">{gig.packages[key].price.toLocaleString('ru-RU')} ₽</td>
                    ))}
                  </tr>
                  <tr className="border-b border-gold/20">
                    <td className="py-3 px-4 text-sm text-muted">{t('gig.deadline')}</td>
                    {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                      <td key={key} className="text-center py-3 px-4 text-sm text-body font-mono">{gig.packages[key].deliveryDays} {t('common.days')}</td>
                    ))}
                  </tr>
                  {gig.packages.premium.features.map((_, fi) => (
                    <tr key={fi} className="border-b border-gold/30">
                      <td className="py-3 px-4 text-sm text-muted">{gig.packages.premium.features[fi]}</td>
                      {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                        <td key={key} className="text-center py-3 px-4">
                          {fi < gig.packages[key].features.length ? <Check size={16} className="text-neon-emerald mx-auto" /> : <span className="text-muted">-</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
                ...
                <div key={review.id} className="space-y-3">
                  ...
                  {review.reply && (
                    <div className="ml-11 pl-4 border-l-2 border-gold/30">
                      <p className="text-xs text-gold mb-1">{t('gig.freelancer_reply')}</p>
            ...
            <div className="card p-6 space-y-5">
              <div className="flex rounded-xl overflow-hidden border border-gold/30">
                {(Object.keys(gig.packages) as PackageKey[]).map((key) => (
                  <button key={key} onClick={() => setActivePackage(key)}
                    className={`flex-1 py-3 text-sm font-medium text-center transition-all cursor-pointer ${activePackage === key ? 'bg-gold/10 text-gold border-b-2 border-gold' : 'text-muted hover:text-body'}`}>
                    {packageLabels[key]}
                  </button>
                ))}
              </div>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-gold font-mono">{pkg.price.toLocaleString('ru-RU')} ₽</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted">
                  <Clock size={14} />
                  <span>{t('gig.deliveryTime')}: <span className="text-body font-mono">{pkg.deliveryDays} {t('common.days')}</span></span>
                </div>
                <div className="space-y-2 pt-2">
                  {pkg.features.map((f) => <div key={f} className="flex items-center gap-2"><Check size={14} className="text-neon-emerald flex-shrink-0" /><span className="text-sm text-body">{f}</span></div>)}
                </div>
              </div>
              <Button variant="primary" size="lg" className="w-full">{t('gig.orderNow')} {pkg.price.toLocaleString('ru-RU')} ₽</Button>
            ...
            <div className="card p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="bg-gold/10 rounded-xl p-3 border border-gold/30">
                  ...
                  <p className="text-xs text-muted">{t('gig.completed_orders')}</p>
                </div>
                <div className="bg-gold/10 rounded-xl p-3 border border-gold/30">
                  <p className="text-lg font-bold text-neon-emerald font-mono">{gig.freelancer.successRate}%</p>
                  <p className="text-xs text-muted">{t('gig.success_rate')}</p>
                </div>
              </div>
              <div className="text-xs text-muted"><span>{t('gig.avg_response')} </span><span className="text-body">{gig.freelancer.responseTime}</span></div>
