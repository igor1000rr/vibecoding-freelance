  ...
  const tabsData = [
    {
      ...
      content: (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {userGigs.length > 0 ? userGigs.map((gig, i) => <GigCard key={gig.id} gig={gig} index={i} />) : gigs.slice(0, 3).map((gig, i) => <GigCard key={gig.id} gig={gig} index={i} />)}
        </div>
      ),
    },
    {
        ...
        <div className="space-y-6">
          {reviews.map((review) => (
            <div key={review.id} className="card p-5 space-y-3">
              ...
              {review.reply && (
                <div className="pl-4 border-l-2 border-gold/30"><p className="text-xs text-gold mb-1">{t('gig.freelancer_reply')}</p><p className="text-sm text-muted">{review.reply}</p></div>
              )}
            </div>
          ))}
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24 md:pb-8">
      ...
      <div className="mt-8 card p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
          ...
          <div className="bg-gold/10 rounded-xl p-4 text-center border border-gold/30">
            <div className="flex items-center justify-center gap-1"><Clock size={16} className="text-gold" /><p className="text-lg font-bold text-heading">{user.responseTime}</p></div>
            <p className="text-xs text-muted mt-1">{t('profile.responseTime')}</p>
          </div>
          <div className="bg-gold/10 rounded-xl p-4 text-center border border-gold/30">
            <div className="flex items-center justify-center gap-1"><TrendingUp size={16} className="text-gold" /><p className="text-lg font-bold text-heading">{user.rating}</p></div>
            <p className="text-xs text-muted mt-1">{t('profile.rating')}</p>
          </div>
        </div>
      </div>
